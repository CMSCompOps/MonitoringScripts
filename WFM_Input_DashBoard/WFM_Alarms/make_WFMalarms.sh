...
while read site ; do
   ...
   # fetching site information numbers of the last 24 hours 
	...
	for nb_entries in `echo 1 4 32 96` ; do
        ...
        if [ $(cat part.dat | awk '{sum +=$4} END {res=(sum<=10); print res}') -eq 1 ] ; then NEW_ALARM[$index]="OK"; fi
        if [ $(tail -n 1 part.dat | awk -v tier="$siteTier" '{res=($2=="on" || (tier=="T1") ); print res}') -eq 0 ] ; then NEW_ALARM[$index]="UNDEF";fi
	...
	if [ $firstSite -eq 1 ] ; then firstSite=0 ; else  echo "," >> $OUTPUTJSON; fi
