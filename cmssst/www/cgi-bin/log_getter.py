...
def lget_maindvi_fts(cfg, docs):
    ...
    # ################################################################ #
    LGET_FTSDASHB = "https://monit-grafana.cern.ch/d/000000420/fts-transfers-30-days?orgId=20&from=%d000&to=%d000&var-group_by=endpnt&var-vo=cms&var-src_country=All&var-dst_country=All&var-src_site=All&var-dst_site=All&var-fts_server=All&var-bin=$__auto_interval_bin&var-filters=%s|=|%s"
    #
    siteRegex = re.compile(r"T\d_[A-Z]{2,2}_\w+")
        ...
        if ( True ):
            ...
            for source in sorted( srcDocs.keys() ):
                ...
                if ( srcDocs[source]['status'] != "unknown" ):
                    ...
                    eTIS = sTIS + cfg['period']
                    myFile.write(("<TR>\n   <TD COLSPAN=" + "\"2\"><A HREF=" +
                                  "\"%s\"><I>Link to transfers in Grafana FT" +
                                  "S dashboard</I></A>\n") % (LGET_FTSDASHB %
                        (sTIS, eTIS, "src_hostname", srcDocs[source]['name'])))
                myFile.write("</TABLE>\n\n")
            myFile.write("<P>\n<HR>\n\n")
            #
            ...
            for dest in sorted( dstDocs.keys() ):
                ...
                if ( dstDocs[dest]['status'] != "unknown" ):
                    ...
                    myFile.write(("<TR>\n   <TD COLSPAN=" + "\"2\"><A HREF=" +
                                  "\"%s\"><I>Link to transfers in Grafana FT" +
                                  "S dashboard</I></A>\n") % (LGET_FTSDASHB %
                          (sTIS, eTIS, "dst_hostname", dstDocs[dest]['name'])))
                myFile.write("</TABLE>\n\n")
            myFile.write("<P>\n<HR>\n\n")
            #
