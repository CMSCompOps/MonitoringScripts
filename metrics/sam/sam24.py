...
# Constants
    ...
    OUTPUT_FILE_CORRECTIONS = os.path.join(options.outputDir,"site_avail_sum_POST_REQUEST.txt")
    ...
    for site in sitesfromDashboard:
        for samSummary in samInfo['data']:
            if samSummary['name'] == site:
                ...
                samScoreSites.append(dashboard.entry(date = dateTo.strftime("%Y-%m-%d %H:%M:%S"), name = site, value = siteAvailability, color = siteColor, url = getSuccessrateUrl (site, dateFrom, dateTo), nvalue=siteAvailabilityNum))
