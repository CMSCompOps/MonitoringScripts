...
# Column IDs.
CPUBOUND_COLUMN_ID = 160
MAXUSED_URL = "http://cms-gwmsmon.cern.ch/totalview/json/maxused"

#Output file
OUTPUT_FILE_NAME = os.path.join(sys.argv[1],"cpu.txt")
...
cpuBoundMetric  = getJSONMetricforAllSitesForDate(CPUBOUND_COLUMN_ID, dateStartStr, dateEndStr)
# Get Json file for maxused
response = urllib.urlopen(MAXUSED_URL)
maxUsedData = json.loads(response.read())
newEntries = []
for site in cpuBoundMetric.getSites():
    entry= cpuBoundMetric.getLatestEntry(site)
    try :
