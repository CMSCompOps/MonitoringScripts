...
# Constants:
    ...
    OUTPUT_FILE_NAME = os.path.join(options.outputDir,"successrate_hctest.txt")
    OUTPUT_FILE_CORRECTIONS = os.path.join(options.outputDir,"successrate_hctest_POST_REQUEST.txt")
    HAMMERCLOUD_COLUMN_NUMBER = "135"
        ...
        for hcSummary in hcInfo['summaries']: 
            if hcSummary['name'] == site and site != "unknown":
                ...
                if siteScore is not None:
                	hcScoreSites.append(dashboard.entry(date = dateTo.strftime("%Y-%m-%d %H:%M:%S"), name = site, value = siteScore, color = siteColor, url = getSuccessrateUrl (site, dateFromStr, dateToStr)))
    #print str(hcScoreSites)
    if len(hcScoreSites) >  noNa: 
        OutputFile = open(OUTPUT_FILE_NAME, 'w')
        correctionOutputFile = open(OUTPUT_FILE_CORRECTIONS, 'a')
        startDateStr = (dateFrom+ timedelta(days=1)).replace(hour=0, minute=0, second=1, microsecond=0).strftime("%Y-%m-%d %H:%M:%S")
        endDateStr = (dateFrom + timedelta(days=1)).replace(hour=23, minute=59, second=59, microsecond=0).strftime("%Y-%m-%d %H:%M:%S")
        for site in hcScoreSites:
            if site.name != "unknown":
                OutputFile.write(str(site) + '\n')
                correctionOutputFile.write(("\t".join([startDateStr, endDateStr, str(HAMMERCLOUD_COLUMN_NUMBER), site.name, str(site.value), site.color, site.url, "nvalue=0"]))+"\n")
